...
from modules import UNet, EMA  # Ensure these are in modules.py
import logging
...
def train(args):
    ...
    model = UNet(c_in=1, c_out=1, time_dim=64, base_dim=16, device=device).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, airfoil in enumerate(pbar):
            train_coords = airfoil['train_coords_y'].float().to(device).unsqueeze(1)
        ...
        if epoch % 100 == 0:
            ...
            ema_sampled_images = diffusion.sample(ema_model, n=5)  # Use cl instead of labels
            #plot_images(sampled_images)
            save_images(sampled_images, airfoil_x, os.path.join("results", args.run_name, f"{epoch}.jpg"))
            save_images(ema_sampled_images, airfoil_x, os.path.join("results", args.run_name, f"{epoch}_ema.jpg"))
...
def launch():
    ...
    parser.add_argument('--project', type=str, default='airfoil_diffusion')
    parser.add_argument('--run_name', type=str, default="unconditional_run_1")
