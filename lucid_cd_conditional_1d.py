...
def save_images_conditional(airfoils,airfoil_x, path, conditioning, num_cols=4):
    ...
    for i in range(num_airfoils):
        ...
        ax.set_title(f'Airfoil {i+1}, {cd_string}')
        ax.set_aspect('equal')
        ax.axis('off')
...
def train(args):
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.75, patience=20, verbose=True)
    l1 = nn.L1Loss()
    ...
    for epoch in range(args.epochs):
        ...
        for i, airfoil in enumerate(pbar):
            ...
            cl = cl + 1
            cd = cd + 1
            conditioning = torch.cat([cd], dim=1)
        ...
        if epoch % 100 == 0:
            fitness = torch.linspace(0, 2, 5).unsqueeze(1).to(device)
            combined = torch.cat([fitness], dim=1)
...
def launch():
    ...
    parser.add_argument('--run_name', type=str, default="lucid_cd_standardized_run_1")
