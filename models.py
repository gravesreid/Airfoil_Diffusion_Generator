...
class AirfoilDiffusionLatent(nn.Module):
    def __init__(self,latent_size,in_channels,out_channels,cl_mean=0, cl_std=1, cd_mean=0, cd_std=1, conditioning=True, time_embedding_dim=256,timesteps=1000,base_dim=32,dim_mults= [1, 2, 4, 8], device="cuda"):
        super().__init__()
        ...
        self.model=Unet(timesteps,time_embedding_dim,in_channels,out_channels,base_dim,dim_mults, cl_mean=cl_mean, cl_std=cl_std, cd_mean=cd_mean, cd_std=cd_std).to(device)
    ...
    def forward(self, x, noise, cl_cd, conditioning):
        # Combine Cl and Cd into a single tensor
        if conditioning == True:
            cond = cl_cd
        else:
            cond = torch.zeros(x.shape[0], 2, device=x.device)
    ...
    def _forward_diffusion(self, x_0, t, noise, cond):
        ...
        q = alpha_scaled * x_0 * (1 + 0 * Cl) + one_minus_alpha_scaled * noise * (1 + 0 * Cd)
...
class AirfoilDiffusion(nn.Module):
    def __init__(self,airfoil_dim,in_channels,out_channels,cl_mean=0, cl_std=1, cd_mean=0, cd_std=1, conditioning=True, time_embedding_dim=256,timesteps=1000,base_dim=32,dim_mults= [1, 2, 4, 8], device="cuda"):
        super().__init__()
    ...
    def sampling(self, n_samples, cl_cd_values, device="cuda"):
        # Initialize noise
        sample = torch.randn(n_samples, self.in_channels, self.airfoil_dim, device=device)
        #print("sample shape in sampling: ", sample.shape)
        all_samples = [sample]
