...
for i, filename in enumerate(generated_files):
    ...
    try:
        ...
        continue
# Convert lists to numpy arrays and flatten the coordinates for PCA
...
principal_components = pca.fit_transform(standardized_coords)

# Plot the PCA components
plt.figure(figsize=(10, 7))
for label, color in zip(['UIUC', 'Generated'], ['red', 'blue']):
    plt.scatter(
        principal_components[labels == label, 0],
        principal_components[labels == label, 1],
        alpha=0.5,
        label=label,
        color=color
    )
...
plt.title('PCA of Airfoil Coordinates')
plt.legend()
plt.grid()
plt.show()
...
# Perform PCA
pca = PCA(n_components=2)
principal_components = pca.fit_transform(standardized_y_coords)
...
plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.title('PCA of Airfoil Y-Coordinates')
plt.legend()
plt.grid()
plt.show()
