...
diffusion_path = "/home/reid/Projects/Airfoil_Diffusion/conditional_airfoil_diffusion/models/lucid_cd_run_1/best_model.pt"
...
if os.path.exists(pkl_save_path):
    ...
    with open(pkl_save_path, 'rb') as f:
        ...
        cd_values = loaded_data['cd_values']
    ...
    print(f'min_cd: {min_cd}')
    max_cd = max(uiuc_cd_values)
    print(f'max_cd: {max_cd}')
    cd_range = torch.linspace(min_cd, max_cd, 20).unsqueeze(1).to(device)
...
def make_hist_and_boxplot(error_list, title_fontsize=22, label_fontsize=20, boundary_thickness=2):
    ...
    axs[0].set_title('CD Difference Histogram', fontsize=title_fontsize)
    ...
    axs[1].set_title('CD Difference Boxplot', fontsize=title_fontsize)
...
def plot_conditioning_scatter(input_values, output_values, title_fontsize=22, label_fontsize=20, boundary_thickness=2):
    ...
    axs.set_xlabel('Input CD Value', fontsize=label_fontsize)
    axs.set_ylabel('Output CD Value', fontsize=label_fontsize)
...
# Call the plotting function with the calculated errors
gen_cd_values = [coef['CD'][0] for coef in gen_coefficients]
plot_conditioning_scatter(cd_values, gen_cd_values, title_fontsize=22, label_fontsize=20, boundary_thickness=2)

# function to plot airfoil contours for generated and uiuc airfoils, with range of cl values
def plot_uiuc_and_gen_contours(uiuc_coordinates, gen_y_coords_list, uiuc_cd_values, gen_cd_values, uiuc_names, uiuc_max_thickness, uiuc_max_camber, gen_max_thickness, gen_max_camber, airfoil_x, cd_range = np.linspace(.001, 0.01, 5), title_fontsize=22):
    # select airfoils for each cl value from gen and uiuc airfoils
    for i, cd in enumerate(cd_range):
        # get closest cl value from gen airfoils
        gen_cd_values = np.array(gen_cd_values)
        gen_cd_diff = np.abs(gen_cd_values - cd)
        gen_cd_index = np.argmin(gen_cd_diff)
        gen_y_coords = gen_y_coords_list[gen_cd_index]
        gen_y_coords = smooth_y_coords(gen_y_coords, method='moving_average', window_size=3)
        
        # get closest cl value from uiuc airfoils
        uiuc_cd_values = np.array(uiuc_cd_values)
        uiuc_cd_diff = np.abs(uiuc_cd_values - cd)
        uiuc_cd_index = np.argmin(uiuc_cd_diff)
        uiuc_coords = uiuc_coordinates[uiuc_cd_index].squeeze()
        ...
        axs[0].set_title(
            f'UIUC {uiuc_names[uiuc_cd_index].item()},\n'
            f'CD: {uiuc_cd_values[uiuc_cd_index].item():.4f} \n',
        ...
        axs[1].set_title(
            f'Generated Airfoil, \n'
            f'CD: {gen_cd_values[gen_cd_index]:.4f} \n'
            , fontsize=title_fontsize
                )
...
plot_uiuc_and_gen_contours(uiuc_coordinates_list, y_coords_list, uiuc_cd_values, gen_cd_values, uiuc_names, uiuc_max_thickness, uiuc_max_camber, gen_max_thickness, gen_max_camber, airfoil_x, cd_range=np.linspace(0.001, 0.01, 5), title_fontsize=22)
...
sc = axs.scatter(latent_tsne[:, 0], latent_tsne[:, 1], c=cd_values, cmap='viridis', label='Generated Airfoils', alpha=0.5)
