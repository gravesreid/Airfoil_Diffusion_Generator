...
def save_images_conditional(airfoils,airfoil_x, path, conditioning, num_cols=4):
    ...
    camber = conditioning[:,0].cpu().numpy()
    print(f'camber shape: {camber.shape}')
    ...
    for i in range(num_airfoils):
        ...
        max_camber_string = f'max_camber={camber[i]:.2f}'
...
with open(uiuc_path, 'rb') as f:
    ...
    # print the keys
    print(uiuc_data.keys())
    uiuc_camber_mean = uiuc_data['uiuc_max_camber_mean']
    uiuc_camber_std = uiuc_data['uiuc_max_camber_std']
...
def train(args):
    ...
    model = Unet1DConditional(12, cond_dim=1, dim_mults=(1, 2, 4), channels=2, dropout=0.).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, airfoil in enumerate(pbar):
            ...
            max_camber = normalize_conditioning_values(max_camber, uiuc_min_camber, uiuc_max_camber)
            # standardize the conditioning
            #max_camber = standardize_conditioning_values(max_camber, uiuc_camber_mean, uiuc_camber_std)
            # shift mean away from 0
            max_camber = max_camber + 2
            conditioning = max_camber.float()
        ...
        if epoch % 100 == 0:
            cm = torch.linspace(0, .5, 5).unsqueeze(1).to(device)
            sampled_images = diffusion.sample(batch_size=5, conditioning=cm)
            save_images_conditional(sampled_images, airfoil_x, os.path.join("results", args.run_name, f"{epoch}.jpg"), cm)
...
def launch():
    ...
    parser.add_argument('--run_name', type=str, default="lucid_camber_normalized_run_1")
