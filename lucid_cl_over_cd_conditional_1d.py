...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, airfoil in enumerate(pbar):
            ...
            cd = cd + 10
            fitness = cl/cd
            conditioning = torch.cat([fitness], dim=1)
        ...
        if epoch % 100 == 0:
            fitness = torch.linspace(7, 13, 5).unsqueeze(1).to(device)
...
def launch():
    ...
    parser.add_argument('--run_name', type=str, default="lucid_cl_over_cd_run_2")
